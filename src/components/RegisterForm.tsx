...
function RegisterForm({previousStep}:propType) {
  ...
  return (
    <div className='flex flex-col items-center justify-center min-h-screen px-6 py-10 bg-white relative'>
        ...
        <p className='text-gray-600 mb-8 flex items-center'>Join SnapKart Today <Leaf className='w-5 h-5 text-green-600'/></p>
        <motion.form initial={{
          opacity:0
        }}
        animate={{
...
<div className="relative">
  ...
  <Mail className="absolute left-3 top-1/2 -translate-y-1/2 w-5 h-5 text-gray-400" />
    <input type="text"placeholder="Your Email"className="w-full border border-gray-400 rounded-xl py-3 pl-10 pr-4 text-gray-800 focus:ring-2 focus:ring-green-500 focus:outline-none"onChange={(e) => setEmail(e.target.value)}
    value={email}
  />
</div>
  <div className="relative">
    <Lock className="absolute left-3 top-1/2 -translate-y-1/2 w-5 h-5 text-gray-400" />
    <input type={showPassword?"text":"password"} placeholder='Your Password'className="w-full border border-gray-400 rounded-xl py-3 pl-10 pr-4 text-gray-800 focus:ring-2 focus:ring-green-500 focus:outline-none"onChange={(e) => setPassword(e.target.value)}
    value={password}
  />
  {
...
{
  (()=>{
  const formValidation=name!=="" && password!==""
...
<button className='w-full flex items-center justify-center gap-3 border border-gray-300 hover:bg-gray-50 py-3 rounded-xl text-gray-700 font-medium transition-all duration-200'>
  ...
  Continue with Google
    ...
    <p className='cursor-pointer text-gray-600 mt-6 text-sm flex items-center gap-1'>Already have an account ?<LogIn className='w-4 h-4'/><span className='text-green-500'>Sign in</span></p>
    </div>
  )
}
